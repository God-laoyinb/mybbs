	...
	<form enctype="multipart/form-data" action="" method="post" class="form form-horizontal" id="form-article-add">
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="" id="title" name="" autocomplete="off">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<span class="select-box">
					<select id="blockSelect" name="" class="select">
						<option value="0">--请选择--</option>
						<c:forEach items="${blockList}" var="block" varStatus="status">
							<option value="${status.index+1}">${block.blockName}</option>
						</c:forEach>
					</select>
				</span> 
			</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<textarea id="content" name="" cols="" rows="" class="textarea" style="width:700px; height:700px" placeholder="说点什么...最少输入10个字符"></textarea>
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				<button onClick="save();" class="btn btn-primary radius" type="button"><i class="Hui-iconfont">&#xe632;</i> 发布　</button>
...
function validate(){//验证输入的准确性
	 var blockName; 
	 if(0==$("#blockSelect").val()){
		blockName = '';
	 } else {
		blockName = $("#blockSelect option:selected").text(); 
	 }
	var res = true;
	...
	if(blockName==''){
		layer.msg('请选择版块名称',{icon:0,time:2000});
		res = false;
	}
	if(title==''){
		layer.msg('请输入主题',{icon:0,time:2000});
		res = false;
	}
...
function save(){
	...
	if(!validate()){
		...
		$.ajax({
			type:"get",
			url:"${pageContext.request.contextPath}/Posting/add",
			dataType:"json",
			async:false,
			data:{"title":title,"content":content,"blockName":blockName,"imgAddress":fileName},
			success:function(data){
				if(data.res==1){
					window.parent.layer.msg('发帖成功',{icon:1,time:2000});
					layer_close();
...
$(function(){
	...
	var uploader = WebUploader.create({
		...
		// 文件接收服务端。
		server: '${pageContext.request.contextPath}/Posting/uploader',
